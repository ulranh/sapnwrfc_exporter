	...
	for _, metric := range config.TableMetrics {
		...
		go func(metric *tMetricInfo, systems systemsInfo) {
			...
			metricsC <- metricData{
				...
				stats:      config.collectSystemsMetric(systems, metric),
			}
...
func (config *Config) collectSystemsMetric(systems systemsInfo, metric metricInter) []statData {
	...
	for _, system := range systems {
		...
		go func(metric metricInter, system *systemInfo) {
			...
			metricC <- config.collectSystemMetric(system, metric)
		}(metric, system)
	}
...
func (config *Config) collectSystemMetric(system *systemInfo, metric metricInter) []statData {
